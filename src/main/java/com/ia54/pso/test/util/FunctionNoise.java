...
public class FunctionNoise implements FunctionPSO {
	...
	public double applyAsDouble(Float x, Float y) {
		...
		return noise(x * frequency, y * frequency, 0);
	}

	@Override
	public float[] interval() {
		float d[] = {50,150,50,150};
		return d;
	}
    ...
    public static double noise(double d, double e, double z) {
        ...
        d -= Math.floor(d);                                // FIND RELATIVE X,Y,Z
        e -= Math.floor(e);                                // OF POINT IN CUBE.
        z -= Math.floor(z);
        double u = fade(d),                                // COMPUTE FADE CURVES
               v = fade(e),                                // FOR EACH OF X,Y,Z.
               w = fade(z);
        int A = p[X  ]+Y, AA = p[A]+Z, AB = p[A+1]+Z,      // HASH COORDINATES OF
            B = p[X+1]+Y, BA = p[B]+Z, BB = p[B+1]+Z;      // THE 8 CUBE CORNERS,

        return lerp(w, lerp(v, lerp(u, grad(p[AA  ], d  , e  , z   ),  // AND ADD
                                       grad(p[BA  ], d-1, e  , z   )), // BLENDED
                               lerp(u, grad(p[AB  ], d  , e-1, z   ),  // RESULTS
                                       grad(p[BB  ], d-1, e-1, z   ))),// FROM  8
                       lerp(v, lerp(u, grad(p[AA+1], d  , e  , z-1 ),  // CORNERS
                                       grad(p[BA+1], d-1, e  , z-1 )), // OF CUBE
                               lerp(u, grad(p[AB+1], d  , e-1, z-1 ),
